...
## Corpus Check
- 12 files · ~47,741 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 146 nodes · 387 edges · 12 communities detected
...
- `build_slide_3()` --calls--> `set_white_background()`  [INFERRED]
  scripts\builders.py → scripts\helpers.py
- `build_slide_3()` --calls--> `add_master_chrome()`  [INFERRED]
  scripts\builders.py → scripts\helpers.py
...
Cohesion: 0.19
Nodes (33): build_slide_1(), build_slide_10(), build_slide_11(), build_slide_12(), build_slide_13(), build_slide_14(), build_slide_15(), build_slide_2() (+25 more)

### Community 1 - "Community 1"
Cohesion: 0.17
Nodes (28): add_line(), add_oval(), add_rect(), add_transparent_text_image(), cm(), emphasis_spans(), hex_to_rgb(), hex_to_rgba() (+20 more)
...
Cohesion: 0.24
Nodes (11): _fallback_summarize_levers(), parse_impact_pdf(), _parse_impact_pdf_fitz(), _parse_impact_pdf_pdfplumber(), Résumé mécanique de secours (≤ max_len caractères) utilisé quand     l'IA ne fou, Résumé mécanique de secours (≤ max_len caractères) utilisé quand     l'IA ne fou, Résumé mécanique de secours (≤ max_len caractères) utilisé quand     l'IA ne fou, Parse un fichier d'analyse d'impact PDF et retourne la même structure. (+3 more)

### Community 4 - "Community 4"
Cohesion: 0.32
Nodes (7): _extract_raw_levers(), _first_sentence(), Extrait les données brutes de la table RESSORTS_CHANGE.      Retourne une liste, Extrait les données brutes de la table RESSORTS_CHANGE.      Retourne une liste, Extrait les données brutes de la table RESSORTS_CHANGE.      Retourne une liste, Résume les actions de la table RESSORTS_CHANGE en une chaîne structurée     pour, _summarize_levers()
...
Cohesion: 0.33
Nodes (6): build_slide_3(), add_teardrop(), Larme (teardrop). En python-pptx : MSO_SHAPE.TEAR (rotation à appliquer pour poi, Larme (teardrop). En python-pptx : MSO_SHAPE.TEAR (rotation à appliquer pour poi, Larme (teardrop). En python-pptx : MSO_SHAPE.TEAR., Larme (teardrop). En python-pptx : MSO_SHAPE.TEAR.
...
Cohesion: 0.4
Nodes (5): parse_impact_file(), Parse un fichier d'analyse d'impact PPTX et retourne une liste de     dictionnai, Parse un fichier d'analyse d'impact PPTX et retourne une liste de     dictionnai, Parse un fichier d'analyse d'impact PPTX et retourne une liste de     dictionnai, Parse un fichier d'analyse d'impact PPTX et retourne une liste de     dictionnai
...
Cohesion: 0.4
Nodes (5): generate_impact_content(), Fonction principale : parse le fichier d'analyse d'impact et retourne     la lis, Fonction principale : parse le fichier d'analyse d'impact et retourne     la lis, Fonction principale : parse le fichier d'analyse d'impact et retourne     la lis, Fonction principale : parse le fichier d'analyse d'impact et retourne     la lis

### Community 11 - "Community 11"
Cohesion: 0.4
Nodes (5): build_layout15_slides(), Convertit la liste de populations en entrées content.json pour le Layout 15., Convertit la liste de populations en entrées content.json pour le Layout 15., Convertit la liste de populations en entrées content.json pour le Layout 15., Convertit la liste de populations en entrées content.json pour le Layout 15.

## Knowledge Gaps
- **70 isolated node(s):** `Retourne le texte brut d'un champ enrichi, sans emphases inline.`, `Dispatcher : appelle le bon builder selon layout_n (1..11).`, `Crée une présentation vierge au bon format.`, `Trouve le logo depuis un chemin explicite ou les dossiers d'assets locaux.`, `Remove PowerPoint theme effects that can add shadows by default.` (+65 more)
...
- **Why does `generate_impact_content()` connect `Community 10` to `Community 2`, `Community 3`, `Community 4`, `Community 8`, `Community 9`, `Community 11`?**
  _High betweenness centrality (0.075) - this node is a cross-community bridge._
- **Why does `build_slide()` connect `Community 2` to `Community 0`?**
